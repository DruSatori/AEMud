...
/*static int num_arg; */
extern char *string_print_formatted (int , char *, int, struct svalue *);
extern char *break_string (char *, int, struct svalue *, int);
...
f_break_string(int num_arg)
{
	...
	case 0:
		...
		(command_giver->interactive->screen_width != 0))
	    {
	        width = (command_giver->interactive->screen_width - 2);
	    }
	    else
	    {
		width = 78;
	...
	str = break_string(arg[0].u.string, width,
	    (num_arg > 2 ? &arg[2] : (struct svalue *)0),
	    (num_arg > 3 ? arg[3].u.number : 0));
