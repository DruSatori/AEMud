...
unswap_svalue(struct buffer *buf, struct svalue *arg)
{
	...
	if (arg->type == T_OBJECT && (arg->u.ob->flags & O_DESTRUCTED))
	    free_svalue(arg);
	if (!(arg->type &
	      (T_OBJECT | T_NUMBER | T_FLOAT | T_ARRAY | T_MAPPING | T_FUNCTION)))
	{
	    (void)printf("SWAP ERROR buf=%lx\n", (unsigned long) buf);
    ...
    case T_MAPPING:
	{
	    ...
	    size = arg->u.number;
	    arg->u.map = allocate_map((short)size);
	    for (i = 0; i < size; i++)
	    {
...
try_to_swap(void)
{
	...
	return;
    swaps = 0;
    if (swap_ob && swap_prog)
    {
	while (swap_ob != obj_list && swap_prog != prog_list)
	{
	    obj_time = (used_memory < max_swap_memory) ?
		max_swap_time : min_swap_time;

	    if (used_memory >= min_swap_memory)
	    {
		if (swap_ob->time_of_ref > swap_prog->time_of_ref)
		{
		    if (current_time - swap_prog->time_of_ref < obj_time)
		    {
			swap_time = swap_prog->time_of_ref + obj_time - 1;
			break;
		    }
		...
		{
		    if (current_time - swap_ob->time_of_ref < obj_time)
		    {
			swap_time = swap_ob->time_of_ref + obj_time - 1;
			break;
		    }
...
#ifdef MAX_CONSECUTIVE_SWAPS
		if (++swaps > MAX_CONSECUTIVE_SWAPS)
		    break;
#endif /* MAX_CONSECUTIVE_SWAPS */
	    }
	    else
	    {
#ifdef DO_CLEANUP
		if (current_time - clean_up_time < min_swap_time)
		    break;
		for (ob = swap_ob; ob != obj_list; ob = ob->prev_all)
		{
		    if (current_time - ob->time_of_ref < obj_time)
			break;
		    if (ob->flags & O_WILL_CLEAN_UP)
		    {
			try_clean_up(ob);
